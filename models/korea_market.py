...
class KoreaMarketDay(BaseModel):
    """
    국내 장 운영 정보 (하루)
    """
...
class KoreaMarketResult(BaseModel):
    """
    국내 장 운영 정보
    """

    @property
    def today(self) -> KoreaMarketDay:
        return KoreaMarketDay(
            self.data["today"]
        )

    @property
    def previous_business_day(self) -> KoreaMarketDay:
        return KoreaMarketDay(
            self.data["previousBusinessDay"]
        )

    @property
    def next_business_day(self) -> KoreaMarketDay:
        return KoreaMarketDay(
    ...
    def __str__(self):
        return f"Korea Market ({self.today.date})"